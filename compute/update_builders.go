...
//	_, err = service.Update(ctx, "my-vm", vm)
//
...
// ⚠️ VM must be stopped before changing compute profile.
...
// ⚠️ VM must be stopped before changing public IP.
...
// ⚠️ VM must be stopped before removing public IP.
...
// ⚠️ VM must be stopped before changing placement group.
...
// ⚠️ VM must be stopped before removing placement group.
...
func (b *VirtualMachineUpdateBuilder) Apply(ctx context.Context) (*compute.VirtualMachine, error) {
	...
	// Send the update
	updated, err := b.service.Update(ctx, b.name, vm)
	if err != nil {
...
func (b *DiskUpdateBuilder) Apply(ctx context.Context) (*compute.Disk, error) {
	...
	// Send the update
	updated, err := b.service.Update(ctx, b.name, disk)
	if err != nil {
