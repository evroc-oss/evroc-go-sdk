...
func setupWaiter(t *testing.T, response string, statusCode int) (*Client, func()) {
	...
	return NewClientWithParent(restClient, &mockContextProvider{}), server.Close
}
...
func TestWaiters(t *testing.T) {
	...
	t.Run("VM_WaitForReady", func(t *testing.T) {
		...
		defer cleanup()
		if err := client.VirtualMachines().WaitForReady(ctx, "test", 2*time.Second, WithPollingInterval(100*time.Millisecond), WithProgressCallback(func(int, time.Duration) {})); err != nil {
	...
	t.Run("VM_WaitForReady_Timeout", func(t *testing.T) {
		...
		defer cleanup()
		if err := client.VirtualMachines().WaitForReady(ctx, "test", 500*time.Millisecond, WithPollingInterval(100*time.Millisecond)); err == nil {
	...
	t.Run("Disk_WaitForReady", func(t *testing.T) {
		client, cleanup := setupWaiter(t, ready, http.StatusOK)
		defer cleanup()
		if err := client.Disks().WaitForReady(ctx, "test", 2*time.Second, WithPollingInterval(100*time.Millisecond)); err != nil {
			t.Errorf("Failed: %v", err)
		}
	})
	...
	t.Run("HotswapDiskAttachment_WaitForReady", func(t *testing.T) {
		client, cleanup := setupWaiter(t, ready, http.StatusOK)
		defer cleanup()
		if err := client.HotswapDiskAttachments().WaitForReady(ctx, "test", 2*time.Second, WithPollingInterval(100*time.Millisecond), WithExponentialBackoff(100*time.Millisecond, 1*time.Second, 2.0)); err != nil {
			t.Errorf("Failed: %v", err)
		}
	})
}
