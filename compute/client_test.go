...
func TestClient(t *testing.T) {
	...
	if _, err := client.VirtualMachines().Patch(ctx, "test", &compute.VirtualMachine{ApiVersion: builderAPIVersion, Kind: "VirtualMachine"}); err != nil {
	...
	if _, err := NewVirtualMachineBuilder("test2").WithDataDisk(DiskRef("/compute/projects/p/regions/r/disks/data-disk")).WithPlacementGroup(PlacementGroupRef("/compute/projects/p/regions/r/placementGroups/pg")).WithRunning(true).Create(ctx, client.VirtualMachines()); err != nil {
	...
	hotswapReq := NewHotswapDiskAttachmentBuilder("test", VMRef("/compute/projects/p/regions/r/virtualMachines/vm"), DiskRef("/compute/projects/p/regions/r/disks/disk")).Build()
	...
	if _, err := client.Disks().Patch(ctx, "test", &compute.Disk{ApiVersion: builderAPIVersion, Kind: "Disk"}); err != nil {
	...
	if NewClient(client.rest, &mockContextProvider{}) == nil {
		t.Error("NewClient failed")
	}

	if _, err := NewHotswapDiskAttachmentBuilder("test", VMRef("/compute/projects/p/regions/r/virtualMachines/vm"), DiskRef("/compute/projects/p/regions/r/disks/disk")).Create(ctx, client.HotswapDiskAttachments()); err != nil {
	...
	if _, err := client.HotswapDiskAttachments().Patch(ctx, "test", &compute.HotswapDiskAttachment{ApiVersion: builderAPIVersion, Kind: "HotswapDiskAttachment"}); err != nil {
	...
	if _, err := client.PlacementGroups().Patch(ctx, "test", &compute.PlacementGroup{ApiVersion: builderAPIVersion, Kind: "PlacementGroup"}); err != nil {
