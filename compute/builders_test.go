...
func TestVirtualMachineBuilder(t *testing.T) {
	req := NewVirtualMachineBuilder("test-vm").
		...
		WithDataDisk("data-disk").
		WithSize("c1a.m").
		WithVMInstanceType("c1a.l").
		WithSSHKey("ssh-ed25519 KEY1").
		WithSSHKey("ssh-rsa KEY2").
		WithSecurityGroup("sg-1").
		WithSecurityGroup("sg-2").
		WithPublicIP("public-ip").
	...
	// Validate disks (should have short IDs - service layer will resolve them)
	if req.Spec.Disks == nil || len(*req.Spec.Disks) != 2 {
		t.Errorf("Expected 2 disks, got %v", req.Spec.Disks)
	} else {
		disks := *req.Spec.Disks
		// First disk should be boot disk with short ID
		if disks[0].DiskRef != "boot-disk" {
			t.Errorf("Expected boot disk 'boot-disk', got %s", disks[0].DiskRef)
		}
		if disks[0].BootFrom == nil || *disks[0].BootFrom != true {
			t.Error("First disk should have BootFrom=true")
		}
		// Second disk should be data disk with short ID
		if disks[1].DiskRef != "data-disk" {
			t.Errorf("Expected data disk 'data-disk', got %s", disks[1].DiskRef)
		}
	...
	// Validate networking (should have short IDs - service layer will resolve them)
	if req.Spec.Networking == nil {
		t.Error("Networking should not be nil")
	} else {
		// Validate public IP (should have short ID)
		...
		} else if *req.Spec.Networking.PublicIPv4Address.Static.PublicIPRef != "public-ip" {
			t.Errorf("Expected public IP 'public-ip', got %s", *req.Spec.Networking.PublicIPv4Address.Static.PublicIPRef)
		}

		// Validate security groups (should have short IDs)
		if req.Spec.Networking.SecurityGroupSettings == nil {
			...
			if len(sgRefs) != 2 {
				...
				if sgRefs[0] != "sg-1" {
					t.Errorf("Expected security group 'sg-1', got %s", sgRefs[0])
				}
				if sgRefs[1] != "sg-2" {
					t.Errorf("Expected security group 'sg-2', got %s", sgRefs[1])
				}
			}
		}
	}
	...
	// Validate placement group (should have short ID - service layer will resolve it)
	if req.Spec.Placement.PlacementGroupRef == nil {
		t.Error("PlacementGroupRef should not be nil")
	} else if *req.Spec.Placement.PlacementGroupRef != "pg" {
		t.Errorf("Expected placement group 'pg', got %s", *req.Spec.Placement.PlacementGroupRef)
	}
...
func Example_virtualMachineBuilder() {
	...
	vmRequest := NewVirtualMachineBuilder("my-web-server").
		...
		WithBootDisk("my-boot-disk").
		WithDataDisk("my-data-disk").
		WithPublicIP("my-public-ip").
		WithSecurityGroup("web-sg").
		WithSecurityGroup("ssh-sg").
