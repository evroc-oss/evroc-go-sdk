...
func TestRESTMethods(t *testing.T) {
	...
	// Test Put
	if _, err := client.Put(ctx, "/test", map[string]string{"key": "value"}); err != nil {
...
func TestResourceHelpers(t *testing.T) {
	...
	// Test UpdateResource
	if _, err := UpdateResource[*testResource](ctx, client, "/test", req); err != nil {
		t.Errorf("UpdateResource failed: %v", err)
	}

	// Test PatchResource
	if _, err := PatchResource[*testResource](ctx, client, "/test", map[string]string{}); err != nil {
