...
func (c *Client) doWithRetry(ctx context.Context, req *http.Request, retryConfig RetryConfig) (*http.Response, error) {
	...
	for attempt := 0; attempt <= retryConfig.MaxRetries; attempt++ {
		...
		reqClone, err := cloneRequest(req, ctx, attempt != 0)
		if err != nil {
			return nil, err
		}
...
// cloneRequest clones a request, creating a fresh body if needed.
func cloneRequest(req *http.Request, ctx context.Context, isRetry bool) (*http.Request, error) {
	reqClone := req.Clone(ctx)

	// If this is the first request, we can use the original body.
	if !isRetry || req.Body == nil {
		return reqClone, nil
	}

	// A request body can only be retried safely if GetBody is available.
	if req.GetBody == nil {
