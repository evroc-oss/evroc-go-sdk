...
func TestCanRetryEvenIfRequestBodyConsumed(t *testing.T) {
	const requestBody = `canary-string`
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if string(body) != requestBody {
			t.Fatalf("expected body %q, got %q", requestBody, string(body))
		}
	...
	if resp == nil || resp.StatusCode != http.StatusOK {
		t.Fatalf("expected 200 response after retry, got: %v", resp)
	}
}
