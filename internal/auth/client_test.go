...
func TestClient_Options(t *testing.T) {
	...
	if httpClient, err := client.HTTPClient(ctx); err != nil || httpClient == nil {
...
func (m *mockMetrics) RecordTokenRefreshError(duration float64, errType string) {
	m.refreshErrorCalls++
	m.lastErrorType = errType
}
func (m *mockMetrics) RecordInitialAuth(duration float64) { m.authCalls++ }
func (m *mockMetrics) RecordInitialAuthError(duration float64, errType string) {
	m.authErrorCalls++
	m.lastErrorType = errType
}
