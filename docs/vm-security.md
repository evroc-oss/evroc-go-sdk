...
vm := compute.NewVirtualMachineBuilder("accessible-vm").
    WithBootDisk(client.Compute().DiskRef("my-disk")).
    WithVMInstanceType("a1a.xs").
    WithSecurityGroup(sg.Ref()).  // Attach security group
    Build()
```
...
**IMPORTANT: SSH keys cannot be changed after VM creation.** You must add all necessary SSH keys during creation. If you lose access, you'll need to destroy and recreate the VM.

```go
// BAD: This VM is inaccessible - no SSH key provided!
vm := compute.NewVirtualMachineBuilder("locked-vm").
    WithBootDisk(client.Compute().DiskRef("my-disk")).
    WithVMInstanceType("a1a.xs").
    WithSecurityGroup(client.Networking().SecurityGroupRef("ssh-access")).
    Build()

// GOOD: Proper VM with SSH key for authentication
...
**Public IPs** give your VMs internet-accessible addresses. **Placement Groups** control VM distribution across physical hosts within a zone - the `spread` strategy ensures VMs run on different physical servers (max 5 VMs per group), so if one server fails, your other VMs keep running. For full high availability, distribute VMs across multiple zones AND use placement groups within each zone.
...
vm, err := client.Compute().VirtualMachines().Create(ctx,
    compute.NewVirtualMachineBuilder("my-vm").
        WithBootDisk(client.Compute().DiskRef("my-disk")).
        WithVMInstanceType("a1a.xs").
        WithSecurityGroup(client.Networking().SecurityGroupRef("my-sg")).
        WithSSHKey(sshPublicKey).
        WithPublicIP(client.Networking().PublicIPRef("my-public-ip")).
        WithPlacementGroup(client.Compute().PlacementGroupRef("my-pg")).
        Build(),
)
```
...
**IMPORTANT: You can only change public IPs and placement groups when the VM is stopped.**
...
By default, evroc VMs run a standard cloud-init script that configures SSH keys and basic system setup. **If you provide a custom cloud-init script, you are responsible for all VM initialization.** The `evroc-user` account will NOT be created, but the distro's default user (e.g., `ubuntu`) will still exist in the base image without SSH keys configured.

```go
// Default behavior - evroc handles SSH key setup and creates evroc-user
vm := compute.NewVirtualMachineBuilder("standard-vm").
    WithBootDisk(client.Compute().DiskRef("my-disk")).
    WithVMInstanceType("a1a.xs").
    WithSSHKey(sshPublicKey).  // evroc's cloud-init creates evroc-user with this key
    Build()
...
- The `evroc-user` account will NOT be created (but distro default users like `ubuntu` still exist in the image)
...
disk, err = client.Compute().Disks().WaitForReady(ctx, "web-app-disk", 5*time.Minute)

// 5. Create VM with security group, SSH key, and public IP
sshPublicKey := "ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAABAQC..."
vm, err := client.Compute().VirtualMachines().Create(ctx,
    compute.NewVirtualMachineBuilder("web-app-vm").
        WithBootDisk(disk.Ref()).         // Use the disk we created above
        WithVMInstanceType("c1a.m").
        WithSecurityGroup(sg.Ref()).      // Use the security group we created above
        WithSSHKey(sshPublicKey).         // Authentication (immutable!)
        WithPublicIP(publicIP.Ref()).     // Use the public IP we created above
        Build(),
)
...
disk, err = client.Compute().Disks().WaitForReady(ctx, "custom-vm-disk", 5*time.Minute)
if err != nil {
    log.Fatal(err)
}
...
vm, err := client.Compute().VirtualMachines().Create(ctx,
    compute.NewVirtualMachineBuilder("custom-vm").
        WithBootDisk(client.Compute().DiskRef("custom-vm-disk")).       // Using the 50GB disk we created
        WithVMInstanceType("a1a.m").          // 4 vCPUs, 16GB RAM
        WithSecurityGroup(client.Networking().SecurityGroupRef("web-sg")).
...
- The `evroc-user` account will NOT be created (but distro default users like `ubuntu` still exist)
