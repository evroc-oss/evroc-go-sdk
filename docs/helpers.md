...
err := client.Compute().Disks().WaitForReady(ctx, "my-disk", 5*time.Minute)
```

## Customizing Waiter Behavior

Waiters now support customizable polling intervals and progress tracking:

```go
// Default behavior: exponential backoff (2s initial, 30s max)
err := client.Compute().Disks().WaitForReady(ctx, "my-disk", 2*time.Minute)
