...
`
	complexVM := compute.NewVirtualMachineBuilder("sdk-vm-complex").
		WithBootDisk("sdk-boot-disk").
		WithDataDisk("sdk-data-disk").
		WithSize(string(compute.VMSizeC1aM)).
		WithZone("a").
		WithPlacementGroup("sdk-pg-spread").
		WithSSHKey("ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIFeENOwB0QwUEicJGrFxt44yiShgBWzANhpE/5gNw041 user@example.com").
	...
	stoppedVM := compute.NewVirtualMachineBuilder("sdk-vm-stopped").
		WithBootDisk("sdk-data-disk-2").  // Using data disk as boot for demo
		WithSize(string(compute.VMSizeA1aXS)).
		WithZone("a").
	...
	_, err = client.Compute().VirtualMachines().Update(ctx, "sdk-vm-simple", vmToStop)
	if err != nil {
	...
	_, err = client.Compute().VirtualMachines().Update(ctx, "sdk-vm-simple", vmToStart)
	if err != nil {
...
func runHotswapExamples(ctx context.Context, client *evroc.Client) error {
	...
	// Wait for disk
	if err := client.Compute().Disks().WaitForReady(ctx, "sdk-hotswap-disk", 2*time.Minute); err != nil {
