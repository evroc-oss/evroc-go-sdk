...
//	SSH_PUBLIC_KEY="ssh-ed25519 AAAA..." go run main.go  # Create with SSH access
package main
...
func create(ctx context.Context, client *evroc.Client) {
	...
	for i, vm := range vms {
		cloudInit := makeCloudInit(vm.name, lbName, os.Getenv("SSH_PUBLIC_KEY"))
...
// If sshPubKey is non-empty, it is added to the ubuntu user's authorized_keys.
func makeCloudInit(hostname, loadBalancerName, sshPubKey string) string {
	sshBlock := ""
	if sshPubKey != "" {
...
`, sshBlock, hostname, loadBalancerName)
}
