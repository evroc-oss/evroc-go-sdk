...
func createResourcesWithLabels(ctx context.Context, client *evroc.Client) error {
	...
	prodDisk := compute.NewDiskBuilder("label-demo-prod-disk").
	...
	_, err := client.Compute().Disks().Create(ctx, prodDisk)
	if err != nil {
	...
	// Wait for disk
	client.Compute().Disks().WaitForReady(ctx, "label-demo-prod-disk", 2*time.Minute)

	// Create VM with environment label
	prodVM := compute.NewVirtualMachineBuilder("label-demo-prod-vm").
		WithBootDisk("label-demo-prod-disk").
		WithSize("a1a.xs").
		WithZone("a").
		WithLabels(map[string]string{
			"environment": "production",
	...
	devDisk := compute.NewDiskBuilder("label-demo-dev-disk").
	...
	teamIP := networking.NewPublicIPBuilder("label-demo-team-ip").
		WithLabels(map[string]string{
			"team":        "platform",
	...
	teamSG := networking.NewSecurityGroupBuilder("label-demo-team-sg").
		AllowSSH().
		WithLabels(map[string]string{
			"team":    "platform",
			"purpose": "ssh-access",
	...
	bucket := storage.NewBucketBuilder("label-demo-bucket").
		WithLabels(map[string]string{
			"purpose":     "backups",
			"retention":   "30-days",
			"environment": "production",
	...
	if orgID != "" {
		project, err := iam.NewProjectBuilder("label-demo-project", orgID).
			WithDisplayName("Label Demo Project").
			WithLabels(map[string]string{
...
func demonstrateLabelBestPractices(ctx context.Context, client *evroc.Client) error {
	...
	wellLabeledVM := compute.NewVirtualMachineBuilder("label-demo-best-practice-vm").
		WithSize("a1a.xs").
		WithZone("a").
		WithLabels(map[string]string{
