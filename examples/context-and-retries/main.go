...
func demonstrateCustomWaiters() {
	...
	start := time.Now()
	_, err = client.Compute().Disks().WaitForReady(ctx, created.Metadata.Id, 2*time.Minute)
	elapsed := time.Since(start)
	...
	start = time.Now()
	_, err = client.Compute().Disks().WaitForReady(
		ctx,
	...
	start = time.Now()
	_, err = client.Compute().VirtualMachines().WaitForReady(
		ctx,
