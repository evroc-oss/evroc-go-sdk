...
// (see examples/vm-backup-to-storage for a complete example with MinIO SDK).
package main
...
func runBucketExamples(ctx context.Context, client *evroc.Client) error {
	...
	if err != nil {
		...
		updatedBucket, err := client.Storage().Buckets().Update(ctx, "sdk-bucket-simple", bucketToUpdate)
		if err != nil {
...
func runBucketServiceAccountExamples(ctx context.Context, client *evroc.Client) error {
	...
	if err != nil {
		...
		updatedSA, err := client.Storage().BucketServiceAccounts().Update(ctx, "sdk-sa-single", saToUpdate)
		if err != nil {
			log.Printf("   Warning: Update failed (may not be supported): %v", err)
		} else {
			bucketCount := 0
