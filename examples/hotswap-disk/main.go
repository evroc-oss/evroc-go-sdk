...
func main() {
	...
	if err := client.Compute().Disks().WaitForReady(ctx, bootDiskName, diskReadyTimeout); err != nil {
		log.Fatal(err)
	}
	...
	if err := client.Compute().VirtualMachines().WaitForReady(ctx, vmName, vmReadyTimeout); err != nil {
		log.Fatal(err)
	}
	...
	if err := client.Compute().Disks().WaitForReady(ctx, dataDiskName, diskReadyTimeout); err != nil {
		log.Fatal(err)
	}
	...
	if err := client.Compute().HotswapDiskAttachments().WaitForReady(ctx, attachmentName, attachmentReadyTimeout); err != nil {
		log.Fatal(err)
	}
