...
// WaitForReady polls the BackendService until it reaches the Ready condition or the timeout is reached.
func (s *BackendServicesService) WaitForReady(ctx context.Context, name string, timeout time.Duration, opts ...WaiterOption) (*lbtypes.Backendservice, error) {
	config := rest.DefaultWaiterConfig()
	config.Timeout = timeout
	...
	var result *lbtypes.Backendservice
	err := rest.WaitFor(ctx, config, func() (bool, error) {
		svc, err := s.Get(ctx, name)
		if err != nil {
			return false, nil
		}
		if IsBackendServiceReady(svc) {
			result = svc
			return true, nil
		}
		return false, nil
...
// WaitForReady polls the L4Route until it reaches the Ready condition or the timeout is reached.
func (s *L4RoutesService) WaitForReady(ctx context.Context, name string, timeout time.Duration, opts ...WaiterOption) (*lbtypes.L4route, error) {
	config := rest.DefaultWaiterConfig()
	config.Timeout = timeout
	...
	var result *lbtypes.L4route
	err := rest.WaitFor(ctx, config, func() (bool, error) {
		route, err := s.Get(ctx, name)
		if err != nil {
			return false, nil
		}
		if IsL4RouteReady(route) {
			result = route
			return true, nil
		}
		return false, nil
